...
def type_hint2schema_type(type_hint: str):
    ...
    if type_hint == 'str':
        ...
        perror(f'Invalid type "{type_hint}"')
...
def get_columns_for_table(schema_text: str, table_name: str) -> List[dict]:
    ...
    if len(res) != 1:
        perror(f'Invalid table name {table_name}')
    return res[0]['fields']


def parse_query(query_text: str, schema_text: str) -> Tuple[List[dict], List[dict]]:
    """
    ...
    tok, idx = consume_token(tokens, idx)
    if not tok:
        perror('Invalid query: {query_text}\n  Expected statement')
    elif tok.upper() == 'SELECT':
        tok, idx = consume_token(tokens, idx)
        if not tok:
            perror(f'Invalid query: {query_text}\n  Expected token after SELECT')
        if tok == '*':
            all_columns = True
            column_names = []
        else:
...
if __name__ == '__main__':
    ...
    for query in queries:
        print(f'Query: {query}')
        res = parse_sql(query, schema_text)
        pp.pprint(res)
